                            ...
                            @foreach ($departments as $department)
                                <tr>
                                    <td> <div class="action-btn bg-primary ms-2" style="font-size:12px;">
                                                   {{ $department->id }}</div></td>
                                    <td>{{ !empty($department->branch)?$department->branch->name:'' }}</td>
                                    <td>{{ $department->name }}</td>
                                    <td>{{  \Carbon\Carbon::parse($department->created_at)->format('d M Y, g:i A D') }}</td>
                                    <td>{{ \Carbon\Carbon::parse($department->updated_at)->format('d M Y, g:i A D') }}</td>
